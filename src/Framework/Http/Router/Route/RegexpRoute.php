...
use Framework\Http\Massage\ServerRequestInterface;
...
class RegexpRoute implements Route
{
    ...
    {    
        ...
        $pattern = preg_replace_callback('~\{([^\}]+)\}~', function($matches) {
            $argument = $matches[1];
            $replase = $this->tokens[$argument] ?? '[^}]+';
            return '(?P<' . $argument . $replase . ')';
        }, $this->pattern);
        ...
        public function generate($name, array $params = []): ?string
        {
            ...
            $url = preg_replace_callback('~\{([^\}]+)\}~', function ($matches) use (&$arguments)
            {
                ...
                {
                    throw new \InvalidArgumentExteption('Missing parameter "' . $argument . '"');
                }
